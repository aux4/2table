...
class NestedDataFormatter {
  ...
  formatObjectWithArrays(obj, field, columnWidths) {
    ...
    field.group.forEach(subField => {
      ...
      if (ContentTypeStrategy.isArray(value)) {
        ...
        // Regular field - check if it has nested structure
        if (subField.group && (ContentTypeStrategy.isObject(value) || ContentTypeStrategy.isArray(value))) {
          // Object/array with group structure - format recursively
  ...
  formatObjectLine(obj, field, columnWidths) {
    ...
    field.group.forEach((subField, index) => {
      ...
      if (isPreFormatted) {
        // Pre-formatted values (from recursive formatting) are used as-is
