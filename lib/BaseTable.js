...
class BaseTable {
  constructor(data, structure, configuration, includeHeaders = true, lineNumbers = false) {
  ...
  addLineNumbersToData(data) {
    // Add line numbers as a regular field in each data object
    return data.map((item, index) => ({
      '__lineNumber__': index + 1,
      ...item
    }));
  }

  addLineNumberToStructure(structure) {
    // Calculate the width needed for line numbers based on data size
