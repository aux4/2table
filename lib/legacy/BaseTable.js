        ...
        if (Array.isArray(value)) {
          ...
          const nestedCellValues = structureItem.group.map(groupItem => {
            ...
            if (nestedValue === undefined || nestedValue === null) {
              ...
              if (groupItem.group) {
                // Array with structure (like logs with level,message) - create recursive table
                const recursiveTable = new (this.table.constructor)(nestedValue, groupItem.group, this.config[structureItem.key].group[groupItem.key].group, false);
                ...
                // Handle simple arrays like endpoints: ["users", "posts", "comments"] -> "users,posts,comments"
  ...
  buildCells() {
    this.structure.forEach(structureItem => {
      ...
      if (structureItem.group && Array.isArray(this.item[structureItem.key])) {
        ...
        if (this.arrayIndex < arrayValue.length) {
          ...
          const nestedCellValues = structureItem.group.map(groupItem => {
            ...
            if (value === undefined || value === null) {
              ...
              if (groupItem.group) {
                // Array with structure - show only the current arrayIndex item (same logic as dependencies)
                if (this.arrayIndex < value.length) {
                  const currentItem = value[this.arrayIndex];
                  ...
                  value = rawOutput.replace(/\s+$/, '').padEnd(this.config[structureItem.key].group[groupItem.key].width.value(), " ");
                } else {
                  value = "".padEnd(this.config[structureItem.key].group[groupItem.key].width.value(), " ");
                }
              } else {
        ...
        // Check if this specific field contains arrays that need processing at this arrayIndex
        const fieldValue = this.item[structureItem.key];
        const fieldHasArraysForThisIndex = structureItem.group && structureItem.group.some(groupItem => {
          if (!fieldValue) return false;
          const nestedValue = fieldValue[groupItem.key];
          return Array.isArray(nestedValue) && this.arrayIndex < nestedValue.length;
        });

        // For non-array fields (like metrics), only show on first row
        // For fields with arrays (like data containing logs), show when arrays have content at this index
        if (this.arrayIndex === 0 || fieldHasArraysForThisIndex) {
          if (structureItem.group) {
            const value = this.item[structureItem.key];
