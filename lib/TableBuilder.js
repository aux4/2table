...
export class TableBuilder {
  ...
  buildDataRow(item) {
    const dataCells = this.structure.map((field, index) => {
      ...
      return this.createCell(content, field, width, isLastColumn);
    });

    return new TableRow(dataCells);
  }

  createCell(content, fieldDef, width, isLastColumn = false) {
    if (fieldDef.group) {
      // Field has nested structure - use DataFormatter with column width access
      const formattedContent = DataFormatter.format(content, fieldDef, this.columnWidths);
      const alignment = this.getAlignment(content);
