...
export class AsciiRenderer {
   ...
   */
  print() {
    return PerformanceMonitor.timeSync('AsciiRenderer.print', () => {
      ...
      const columnWidths = PerformanceMonitor.timeSync('AsciiRenderer.calculateColumnWidths',
        () => this.calculateColumnWidthsOptimized(columns),
        { columns, rows }
      );

      // Batch row processing for wide tables (optimization for 10+ columns)
      if (columns >= 10) {
        const result = this.renderRowsBatched(rows, columnWidths);
        return result;
      }
      ...
      optimizationUsed: this.dimensions.columns >= 10 ? 'batched' : 'standard'
    });
  }
  ...
  renderRowsBatched(totalRows, columnWidths) {
    return PerformanceMonitor.timeSync('AsciiRenderer.renderRowsBatched', () => {
      const lines = [];
      ...
      totalRows,
      batchSize: Math.max(1, Math.min(10, Math.floor(100 / this.dimensions.columns)))
    });
  }
